<?php
/*
Plugin Name: MTP Ads 101*
Plugin URI: http://mt-performance.net
Description: Creates a new sidebar and a new widget to place your ads on different positions inside your website. *Works only with the new Thesis 2.0.1!
Version: 1.0.1
Author: Toni Medina
...
class mtp_ad extends WP_Widget
{
	...
	public function __construct()
	{
		...
		foreach ($instance as $key => $value)
		{
			...
			{
				add_action('thesis_hook_before_container_logo',array($this,'mtp_ad_thbt'));
			}
			else if ($pat_arr_search == true)
			{
				add_action('thesis_hook_after_container_logo',array($this,'mtp_ad_that'));
			}
			else if ($pbi_arr_search == true)
			{
				add_action('thesis_hook_after_container_byline',array($this,'mtp_ad_thbi'));
			}
			else if ($pbc_arr_search == true)
			{
				add_action('thesis_hook_container_content_container_top',array($this,'mtp_ad_thbc'));
			}
			else if ($pbs_arr_search == true)
			{
				add_action('thesis_hook_before_container_sidebar',array($this,'mtp_ad_thbs'));
			}
			else if ($pap_arr_search == true)
			{
				add_action('thesis_hook_before_container_entries_navigation',array($this,'mtp_ad_thap'),1);
			}
			else if ($pbf_arr_search == true)
			{
				add_action('thesis_hook_container_footer_container_top',array($this,'mtp_ad_thbf'));
			}
			...
			{
				add_action('thesis_hook_after_post_box_wp_loop',array($this,'mtp_ad_thapb'));
			}
			else if ($patb_arr_search == true)
			{
				add_action('thesis_hook_after_post_box_wp_loop',array($this,'mtp_ad_thatb'));
			}
		}
	}
